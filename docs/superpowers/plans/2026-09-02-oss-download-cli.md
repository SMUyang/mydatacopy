...
**Goal:** Add a cross-platform, download-only `oss-download` CLI that safely configures AccessKey credentials and delegates resumable large-file transfers to Alibaba Cloud ossutil 2.x.

**Architecture:** A standard-library Python module validates OSS/local paths and constructs an `ossutil cp` subprocess command. An interactive `configure` subcommand delegates credential storage to the official ossutil wizard; download mode never accepts a Secret as a normal command-line argument. A console-script entry point and no-install launcher provide the same CLI on Linux, macOS, and Windows.

**Tech Stack:** Python 3.10+, argparse, pathlib, getpass, subprocess, unittest, setuptools, Alibaba Cloud ossutil 2.x.

---

### Task 1: Download command and credential surface

**Files:**
- Create: `oss_download.py`
- Create: `oss-download`
...
Cover required `--source`/`--destination`, `oss://` source validation, local-only destination validation, hidden credential input through `getpass`, secret redaction from logs, and construction of v2 `ossutil cp` with `-r`, `-j`, positive-integer `--parallel`, `--checkpoint-dir`, `--include`, `--exclude`, and `--bandwidth-limit`.
...
Use `argparse` subparsers. `download` must reject non-OSS sources and cloud destinations, reject missing destination, and check `shutil.which("ossutil")` unless `--ossutil PATH` is supplied. Use `getpass.getpass` for an optional one-shot credential path; never include a Secret in a subprocess argv or log line. Build the official v2 `ossutil cp` invocation, map `--maxdownspeed` to `--bandwidth-limit`, accept only a positive integer for `--parallel`, and pass explicit download options only. Preserve the subprocess exit code.
...
Document Linux, macOS, and Windows PowerShell examples; `configure`; hidden AccessKey Secret behavior; recursive and single-object downloads; checkpoint resume; concurrency tuning with positive-integer `--parallel`; filters; speed limiting via `--maxdownspeed`/`--bandwidth-limit`; dry-run; log files; local-only destinations; and the download-only safety boundary.
...
Expected: both help surfaces show identical download/configure options and all tests pass.
